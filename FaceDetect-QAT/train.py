...
from data.data_argument import preproc
...
if args.network == "slim":
    ...
    net.apply(torch.quantization.enable_observer)
    net.apply(torch.quantization.enable_fake_quant)
else:
...
if args.resume_net is not None:
    ...
    if args.network == "slim-qat":
        state_dict = torch.load(
            args.resume_net, map_location=torch.device('cpu'))
    else:
