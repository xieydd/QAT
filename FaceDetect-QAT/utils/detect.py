...
# python detect.py --network=slim-qat --cpu
...
def load_model(model, pretrained_path, load_to_cpu):
    ...
    # add ['model'] after pretrained_dict when use qat training
...
if __name__ == '__main__':
    ...
    face_count = 0
    count = 0
    img_dir = '/googol/atlas/public/haitian_RGB_img_label/haitian_RGB'
    result_dir = "./result"
    if not os.path.exists(result_dir):
        ...
        os.mkdir(result_dir)
    for index, img_id in enumerate(os.listdir(img_dir)):
        if img_id == "register":
            continue

        for img_path in os.listdir(os.path.join(img_dir, img_id)):
            if img_path == "1190_dark.jpg":
                continue
            image_path = os.path.join(img_dir, img_id, img_path)
