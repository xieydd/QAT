...
parser.add_argument('--bin_folder',
                    default='./bin/', type=str, help='bin dir path')
...
def load_model(model, pretrained_path, load_to_cpu):
    ...
    if load_to_cpu:
        pretrained_dict = torch.load(
            pretrained_path, map_location=lambda storage, loc: storage)['model']
    else:
...
if __name__ == '__main__':
    ...
    for i, img_name in enumerate(test_dataset):
        ...
        bin_dir = args.bin_folder + img_name.split('/')[-1][:-3] + "bin"
        loc = list(map(float, linecache.getline(bin_dir, 2)[:-1]))
        conf = list(map(float, linecache.getline(bin_dir, 3)[:-1]))
        landms = list(map(float, linecache.getline(bin_dir, 4)[:-1]))

        size = int(len(loc)/4)
        loc = torch.Tensor(loc).view(1, size, -1).to(device)
        conf = torch.Tensor(conf).view(1, size, -1).to(device)
        landms = torch.Tensor(landms).view(1, size, -1).to(device)
        im_width = list(map(int, linecache.getline(bin_dir, 1)[:-1]))[0]
        im_height = list(map(int, linecache.getline(bin_dir, 1)[:-1]))[1]
        # testing scale
