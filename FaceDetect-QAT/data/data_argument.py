...
def PadToSquare(sample, input_size=300):
...
def Resizer(sample, input_size=300):
    image, annots = sample['img'], sample['annot']
    ...
    resized_image = cv2.resize(
        image, (int(rows*input_size / long_side), int(cols*input_size / long_side)))
    resized_image = resized_image * 255

    assert (resized_image.shape[0] == input_size or resized_image.shape[1]
            == input_size), 'resized image size not {}'.format(input_size)
...
class preproc_eval(object):
    ...
    def __call__(self, image, targets):
        ...
        boxes = targets[:, :4].copy()
        # labels = np.zeros((boxes.shape[0], 1))
        # landm = np.zeros((boxes.shape[0], 10))
        ...
        # labels_t = np.expand_dims(labels_t, 1)
        # #targets_t = np.hstack((boxes_t))
        # targets_t = boxes_t
        image = image.astype(np.float32)
        image -= self.rgb_means
        sample = {'img': image, 'annot': boxes}
        sample = Resizer(sample, self.img_dim)
        sample = PadToSquare(sample, self.img_dim)
        image_t, targets_t = sample['img'], sample['annot']
        return image_t, targets_t
